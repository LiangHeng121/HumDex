...
if ! command -v python >/dev/null 2>&1; then
  echo "❌ 当前环境里找不到 python，可尝试检查 conda 环境：__CONDA_ENV__"
  exit 4
fi

if ! python - <<'PY'
import importlib.util
mods = ["zmq", "cv2", "pyrealsense2"]
missing = [m for m in mods if importlib.util.find_spec(m) is None]
if missing:
    raise SystemExit("MISSING:" + ",".join(missing))
PY
then
  echo "❌ 当前 Python 环境缺少依赖（zmq/cv2/pyrealsense2 之一）"
  echo "   建议在 g1 上执行："
  echo "   conda activate __CONDA_ENV__"
  echo "   python -m pip install pyzmq opencv-python"
  echo "   # pyrealsense2 需按你的设备环境安装（Jetson/g1 常用系统包或预编译轮子）"
  exit 5
fi
...
exec python server_realsense_zmq_pub.py __PY_ARGS__
EOF
)
