        ...
        if self.redis_client is not None:
            ...
            if self.left_hand_tracking_dict is not None:
                hand_tracking_data_left = {
                    ...
                    "timestamp": current_time_ms,  # 添加时间戳
                    **self.left_hand_tracking_dict  # 包含所有26个关节的数据
                }
                self.redis_client.set(
                    "hand_tracking_left_unitree_g1_with_hands", 
                    json.dumps(hand_tracking_data_left)
                )
            else:
                # 如果没有数据，发送 inactive 状态
                hand_tracking_data_left = {
                    "is_active": False,
            ...
            if self.right_hand_tracking_dict is not None:
                hand_tracking_data_right = {
                    ...
                    "timestamp": current_time_ms,  # 添加时间戳
                    **self.right_hand_tracking_dict  # 包含所有26个关节的数据
                }
                self.redis_client.set(
                    "hand_tracking_right_unitree_g1_with_hands", 
                    json.dumps(hand_tracking_data_right)
                )
            else:
                # 如果没有数据，发送 inactive 状态
                hand_tracking_data_right = {
    ...
    def run(self):
        ...
        with mjv.launch_passive(
            ...
            while viewer.is_running():
                ...
                # print("kdkasdnasdlknasldnasld")
                # print(smplx_data)
                # print(self.left_hand_is_active)
                # print(self.left_hand_tracking_dict)
                # print(self.right_hand_is_active)
                # print(self.right_hand_tracking_dict)
