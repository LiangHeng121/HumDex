...
def _load_pose_csv_frames(csv_path: str) -> Tuple[List[Dict[str, Any]], float]:
    """
    Load FK-ed pose CSV frames (only hands) for visualization.
    Each frame will contain keys like LeftHand/LeftIndexFinger3/... and RightHand/...
    """
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    if repo_root not in sys.path:
        sys.path.insert(0, repo_root)
    ...
    frames, _meta = load_pose_csv_frames(csv_path, include_body=False, include_lhand=True, include_rhand=True, max_frames=-1)
    return frames, 1.75


def _make_hand_parent_map() -> Dict[str, Optional[str]]:
    """
    Minimal parent map for hand joints (for drawing lines when input is CSV without BVH hierarchy).
    """
    parent: Dict[str, Optional[str]] = {}
    for pfx in ["Left", "Right"]:
        wrist = f"{pfx}Hand"
        parent[wrist] = None
        # thumb: Hand -> ThumbFinger -> ThumbFinger1 -> ThumbFinger2
        parent[f"{pfx}ThumbFinger"] = wrist
        parent[f"{pfx}ThumbFinger1"] = f"{pfx}ThumbFinger"
        parent[f"{pfx}ThumbFinger2"] = f"{pfx}ThumbFinger1"
        # index
        parent[f"{pfx}IndexFinger"] = wrist
        parent[f"{pfx}IndexFinger1"] = f"{pfx}IndexFinger"
        parent[f"{pfx}IndexFinger2"] = f"{pfx}IndexFinger1"
        parent[f"{pfx}IndexFinger3"] = f"{pfx}IndexFinger2"
        # middle
        parent[f"{pfx}MiddleFinger"] = wrist
        parent[f"{pfx}MiddleFinger1"] = f"{pfx}MiddleFinger"
        parent[f"{pfx}MiddleFinger2"] = f"{pfx}MiddleFinger1"
        parent[f"{pfx}MiddleFinger3"] = f"{pfx}MiddleFinger2"
        # ring
        parent[f"{pfx}RingFinger"] = wrist
        parent[f"{pfx}RingFinger1"] = f"{pfx}RingFinger"
        parent[f"{pfx}RingFinger2"] = f"{pfx}RingFinger1"
        parent[f"{pfx}RingFinger3"] = f"{pfx}RingFinger2"
        # pinky
        parent[f"{pfx}PinkyFinger"] = wrist
        parent[f"{pfx}PinkyFinger1"] = f"{pfx}PinkyFinger"
        parent[f"{pfx}PinkyFinger2"] = f"{pfx}PinkyFinger1"
        parent[f"{pfx}PinkyFinger3"] = f"{pfx}PinkyFinger2"
    return parent
...
def main() -> int:
    ...
    ap.add_argument("--bvh_file", type=str, default="", help="(legacy) BVH file path")
    ap.add_argument("--csv_file", type=str, default="", help="Pose CSV path (FK-ed). If provided, ignores BVH hierarchy/end_sites.")
    ap.add_argument("--csv_units", choices=["m", "cm", "mm"], default="m", help="Units of CSV position fields (default: m)")
    ap.add_argument("--csv_apply_bvh_rotation", action="store_true", help="Apply BVH-like axis rotation to CSV pos/quat (convert world coords to GMR convention)")
    ap.add_argument("--csv_geo_xyz_to_bvh_xzy", action="store_true", help="Apply Geo XYZ -> BVH XZY basis change (swap Y/Z) to CSV pos+quat")
    ap.add_argument("--csv_geo_xyz_to_bvh_xzy_pos_only", action="store_true", help="Apply Geo XYZ -> BVH XZY basis change to CSV positions only (swap Y/Z), keep quats unchanged")
    ap.add_argument("--csv_geo_xyz_to_nokov", action="store_true", help="Apply fixed WS_Geo XYZ -> nokov/GMR mapping: (x,y,z)->(y,-x,z)")
    ap.add_argument("--csv_quat_order", choices=["wxyz", "xyzw"], default="wxyz", help="Quaternion order in CSV (default: wxyz)")
    ap.add_argument("--csv_quat_space", choices=["global", "local"], default="global", help="Quaternion space in CSV (default: global)")
    ap.add_argument("--csv_auto_canonical", action="store_true", help="Auto-rotate CSV world axes to canonical (x=fwd,y=left,z=up) using shoulders/spine (frame0)")
    ap.add_argument("--format", choices=["lafan1", "nokov"], default="nokov", help="BVH format (only used with --bvh_file)")
    ...
    ap.add_argument("--end_sites", action="store_true", help="(BVH only) Include End Site points if present")
    ...
    if not args.csv_file and not args.bvh_file:
        print("❌ 需要提供 --csv_file 或 --bvh_file", file=sys.stderr)
        return 2

    csv_mode = bool(args.csv_file)
    if csv_mode:
        data_path = _resolve_any_path(args.csv_file)
        if not os.path.exists(data_path):
            print(f"❌ 找不到 CSV 文件: {args.csv_file}", file=sys.stderr)
            return 2
        frames, _hh = _load_pose_csv_frames(data_path)
        if args.csv_quat_order != "wxyz" or args.csv_quat_space != "global" or args.csv_geo_xyz_to_nokov or args.csv_geo_xyz_to_bvh_xzy or args.csv_geo_xyz_to_bvh_xzy_pos_only or args.csv_apply_bvh_rotation or args.csv_units != "m" or args.csv_auto_canonical:
            from deploy_real.pose_csv_loader import (
                apply_bvh_like_coordinate_transform,
                convert_quat_order,
                default_parent_map_body,
                quats_local_to_global,
                estimate_canonical_rotation_from_frame,
                apply_global_rotation,
                apply_axis_basis_change_xyz_to_xzy,
                apply_axis_basis_change_xyz_to_xzy_pos_only,
                apply_geo_xyz_to_nokov,
            )  # type: ignore
            ...
            if args.csv_geo_xyz_to_nokov and (args.csv_geo_xyz_to_bvh_xzy or args.csv_geo_xyz_to_bvh_xzy_pos_only):
                print("⚠️ 同时设置了 --csv_geo_xyz_to_nokov 与 --csv_geo_xyz_to_bvh_xzy*，将优先使用 --csv_geo_xyz_to_nokov", file=sys.stderr)
            if args.csv_geo_xyz_to_bvh_xzy and args.csv_geo_xyz_to_bvh_xzy_pos_only:
                print("⚠️ 同时设置了 --csv_geo_xyz_to_bvh_xzy 与 --csv_geo_xyz_to_bvh_xzy_pos_only，将优先使用 pos+quat 版本", file=sys.stderr)
            if args.csv_geo_xyz_to_nokov:
                frames = [apply_geo_xyz_to_nokov(fr) for fr in frames]
            elif args.csv_geo_xyz_to_bvh_xzy:
                frames = [apply_axis_basis_change_xyz_to_xzy(fr) for fr in frames]
            elif args.csv_geo_xyz_to_bvh_xzy_pos_only:
                frames = [apply_axis_basis_change_xyz_to_xzy_pos_only(fr) for fr in frames]
            ...
            if args.csv_auto_canonical:
                R = estimate_canonical_rotation_from_frame(frames[0])
                if R is not None:
                    frames = [apply_global_rotation(fr, R) for fr in frames]
    else:
        data_path = _resolve_any_path(args.bvh_file)
        if not os.path.exists(data_path):
            print(f"❌ 找不到 BVH 文件: {args.bvh_file}", file=sys.stderr)
            return 2
        frames, _hh = _load_frames(data_path, args.format)
    if not frames:
        print("❌ BVH 没有帧数据", file=sys.stderr)
        return 2

    # Parse hierarchy once if needed (BVH) or create a minimal hand parent map (CSV)
    parent_map: Dict[str, Optional[str]] = {}
    endsite_offsets_cm: Dict[str, np.ndarray] = {}
    if csv_mode:
        if args.lines:
            parent_map = _make_hand_parent_map()
        # CSV has no End Site
        endsite_offsets_cm = {}
    else:
        if args.lines or args.end_sites or args.print_wrist_to_tips or args.video_out:
            parent_map, endsite_offsets_cm = _parse_bvh_hierarchy_parents_and_endsites(data_path)

    # Print max distances over all frames (requested)
    if args.print_wrist_to_tips:
        try:
            if csv_mode:
                print("[note] CSV 模式不包含 End Site：指尖使用末节关节位置（ThumbFinger2/IndexFinger3/MiddleFinger3/RingFinger3/PinkyFinger3）")
            else:
                print("[note] BVH 模式若包含 End Site：优先用 End Site 指尖；缺失则 fallback 到末节关节位置。")

            # Track max distance and which frame it occurs
            fingers = ["thumb", "index", "middle", "ring", "pinky"]
            max_left = {k: -1.0 for k in fingers}
            max_right = {k: -1.0 for k in fingers}
            arg_left = {k: -1 for k in fingers}
            arg_right = {k: -1 for k in fingers}

            for fi, fr in enumerate(frames):
                dl = _compute_wrist_to_fingertip_distances_m(fr, "left", endsite_offsets_cm)
                dr = _compute_wrist_to_fingertip_distances_m(fr, "right", endsite_offsets_cm)
                for k in fingers:
                    v = float(dl.get(k, -1.0))
                    if v > max_left[k]:
                        max_left[k] = v
                        arg_left[k] = fi
                    v2 = float(dr.get(k, -1.0))
                    if v2 > max_right[k]:
                        max_right[k] = v2
                        arg_right[k] = fi

            print(f"[all_frames] max wrist->fingertip distances (m), total_frames={len(frames)}")
            print("  left : " + ", ".join([f"{k}={max_left[k]:.4f} (frame={arg_left[k]})" for k in fingers]))
            print("  right: " + ", ".join([f"{k}={max_right[k]:.4f} (frame={arg_right[k]})" for k in fingers]))

            # Also print overall max finger per hand
            left_best = max(fingers, key=lambda k: max_left[k])
            right_best = max(fingers, key=lambda k: max_right[k])
            print(f"  left_best : {left_best}={max_left[left_best]:.4f} (frame={arg_left[left_best]})")
            print(f"  right_best: {right_best}={max_right[right_best]:.4f} (frame={arg_right[right_best]})")
        except Exception as e:
        ...
        with tempfile.TemporaryDirectory(prefix="bvh_hand_vid_") as tmpd:
            ...
            for k, i in enumerate(range(vstart, vend)):
                ...
                if args.video_layout == "both":
                    ...
                    fig.suptitle(f"Hands: {os.path.basename(data_path)}", fontsize=12)
                elif args.video_layout == "left":
                    plot_one(ax_l, l_t, l_end_t, f"Left hand frame={i} ({args.video_coords}, x{scale:g})", "tab:blue", zl_mn, zl_mx)
                    fig.suptitle(f"Left hand: {os.path.basename(data_path)}", fontsize=12)
                else:
                    # right
                    plot_one(ax_l, r_t, r_end_t, f"Right hand frame={i} ({args.video_coords}, x{scale:g})", "tab:orange", zr_mn, zr_mx)
                    fig.suptitle(f"Right hand: {os.path.basename(data_path)}", fontsize=12)
                fig.tight_layout()
    ...
    fig.suptitle(f"Hands visualization: {os.path.basename(data_path)}", fontsize=12)
    fig.tight_layout()
