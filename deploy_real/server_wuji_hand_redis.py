...
class WujiHandRedisController:
    ...
    def __init__(
        ...
        if self.use_model:
            ...
            # Optional: tune / disable DexPilot pinch projection thresholds
            try:
            ...
            # IMPORTANT: `retarget_result.robot_qpos` is in Pinocchio DOF order (self.retargeter.optimizer.target_joint_names).
                ...
                if is_active and hand_data_dict is not None:
                    try:
                        ...
                        if self.use_model:
                            ...
                            qpos20 = np.asarray(retarget_result.robot_qpos, dtype=np.float32).reshape(-1)
                        ...
                        if (not self.use_model) and self.debug_pinch and (self._frame_count % self.debug_pinch_every == 0):
                            ...
                            try:
                                ...
                                if opt2 is not None and hasattr(opt2, "robot"):
                                    robot = opt2.robot
                                    qpos20_fk = retarget_result.robot_qpos.astype(np.float64).reshape(-1)
...
def main():
    ...
    print(f"模式: {'model_inference' if args.use_model else 'dexpilot_retarget'}")
    if args.use_model:
