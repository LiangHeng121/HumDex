...
def _load_episode_data(hdf5_file, episode_id, hand_side):
    ...
    Returns:
        qpos: (T, state_dim) array - state_dim is 51 (single) or 71 (both)
...
def get_norm_stats(dataset_path, episode_ids, hand_side: str = "left"):
    ...
    # normalize qpos data
    qpos_mean = all_qpos_data.mean(dim=0, keepdim=True)  # (1, state_dim) - 51 or 71
...
def custom_collate_fn(batch):
    ...
    qpos_batch = torch.stack(qpos, dim=0)  # (batch, 51 or 71)
