...
class DexPilotOptimizer:
    ...
    def __init__(
        ...
        self.last_project_dist = float(project_dist)
        self.last_escape_dist = float(escape_dist)
        self.eta1 = eta1
        self.eta2 = eta2
    ...
    def get_objective_function(
        ...
        # Use fixed projection thresholds (no hand_scale adaptive logic)
        project_dist = float(self.project_dist)
        escape_dist = float(self.escape_dist)
        self.last_project_dist = project_dist
        self.last_escape_dist = escape_dist

        # Update projection indicator based on target vector distances
        target_vec_dist = np.linalg.norm(target_vector[:len_proj], axis=1)
        self.projected[:len_s1][target_vec_dist[0:len_s1] < project_dist] = True
        self.projected[:len_s1][target_vec_dist[0:len_s1] > escape_dist] = False
